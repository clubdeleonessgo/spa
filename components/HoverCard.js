...
export default function HoverRevealCard({ data }) {
    ...
    const onEnter = () => {
        if (enabled && window.innerWidth >= 768) setActive(true);
    };
    const onLeave = () => {
        if (enabled && window.innerWidth >= 768) setActive(false);
    };
    ...
    return (
        ...
        >
            <div
                className={
                    enabled
                        ? `rounded-lg transition-opacity duration-300 ease-out 
            ...
            >
                ...
                <div
                    className={`flex flex-col justify-center xs:justify-start xs:flex-row items-center rounded-r-lg col-span-2
            text-white p-3 xs:p-5 ${height} ${data.color}`}
                >
                    <div className={"flex items-center flex-col w-24"}>
                        ...
                        {data.value > 0 && (
                            <Counter
                                value={data.value}
                                className={"lg:text-3xl sm:text-4xl font-extrabold mt-2"}
                            />
                        )}
                    </div>
                    <ul
                        className={`list-disc text-base pl-1 xs:pl-5 ml-4 ${
                            data.black ? "text-black" : "text-white"
                        }`}
                    >
                ...
                <div className="absolute inset-x-0 bottom-0">
                    <div className="p-2">
                        <div className="rounded-lg bg-black/40 px-2 py-1 text-white font-semibold w-fit">
                            {data.title}
                        </div>
                    </div>
                </div>
